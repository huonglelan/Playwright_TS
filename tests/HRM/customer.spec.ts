...
test.describe("CRM Customer Page - positive case", () => {
  ...
  test("TC_CUST_02 - Tạo Customer (Nhập đầy đủ thông tin)", async ({
    ...
    //   .filter({
    //     has: page.locator("span.text", { hasText: "USD" }),
    //   })
    //   .click();

    //dropdown này làm tương tự với currency
    ...
    //có thể xài
    //  await page.locator(`//a[normalize-space()='Jamaica']`).click();
...
test.describe("Kịch bản Chức năng (UI/Functionality)", () => {
  test('TC_CUST_04 - Kiểm tra "Same as Customer Info" (Billing) ', async ({
    ...
    await expect(
      page.locator("//div[@class='col-md-6']/h4").nth(0)
    ...
    const title = await page
      .locator("//button[@data-id='billing_country']")
      .getAttribute("title");
    console.log(title);
...
test.describe("Kịch bản Thất Bại (Negative - Validation)", () => {
  test('TC_CUST_06 - Bỏ trống trường "Company" ', async ({ page }) => {
    ...
    await expect(page.locator("#company-error")).toHaveText(
  ...
  test('TC_CUST_07 - Cảnh báo "Company đã tồn tại" ', async ({ page }) => {
    ...
    //có thể làm full luồng tạo company -> tạo lại company với tên đã tạo

    //Điền tên Company trùng lặp đó vào ô Company.
    await page.locator("#company").fill("Huong123");
    //click vao o khac
    //có thể dùng blur để ko focus vào ô input
